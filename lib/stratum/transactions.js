    ...
    for (var i = 0; i <= vouts.length - 1; i++) {
        ...
        switch (vouts[i].scriptPubKey.type) {
            ...
            case "cryptocondition":
                // VRSC's reserve-currency/feepool coinbase outputs use
                // OP_CHECKCRYPTOCONDITION with embedded condition data, not an
                // address-derived script - there's no address to rebuild a
                // script from (scriptPubKey.addresses, which "default" below
                // relies on, isn't meaningful here). This is the daemon's own
                // getblocktemplate-proposed coinbase, so the script it already
                // decoded is by definition the consensus-valid one; reproduce
                // it verbatim instead of guessing at a P2PKH reconstruction.
